...
class PickAndPlaceVacuum(Node):
    ...
    # --- 変更箇所: 数値入力メソッドの追加 ---
    def get_user_pose(self, name):
        print(f"\n--- Input Coordinates for {name} ---")
        try:
            x = float(input(f"Enter X for {name}: "))
            y = float(input(f"Enter Y for {name}: "))
            z = float(input(f"Enter Z for {name}: "))
            return [x, y, z, 0.0]
        except ValueError:
            print("Invalid input. Please enter numbers only.")
            return self.get_user_pose(name)

    # --- 変更箇所: 全て入力式に書き換え ---
    def set_pose(self):
        self.home_pose_  = self.get_user_pose("HOME POSITION")
        self.pick_pose_  = self.get_user_pose("PICK POSITION")
        self.place_pose_ = self.get_user_pose("PLACE POSITION")
        print("\nAll coordinates have been set.")
    # ---------------------------------------
